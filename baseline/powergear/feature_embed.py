...
from op_extract import df_graph_visualize2, df_cc_trim, cdfg_fsm_node
...
op_act_3 = ["add", "sub", "mul", "div", "fadd", "fsub", "fmul", "fdiv", "icmp", "fcmp", "and", "or",
            "xor", "shl", "lshr", "dadd", "dmul", "dsub", "ddiv"]  # tracer seq: opnd_1, opnd_2, opnd_out
...
def get_cdfg_fsm_dict(in_dir):
    ...
    with open('{}/final/kernel.txt'.format(in_dir), 'r') as rfile:
        kernel_list = re.split(' ', rfile.readline())
    kernel_list.append(args.kernel_name)
    for kernel in kernel_list:
        if kernel != '':
            with open('{}/{}/cdfg_fsm_node_dict.csv'.format(in_dir, kernel), 'r') as fop:
                rdop = csv.reader(fop)
                next(rdop, None)
                for row in rdop:
                    nodeid, name, rtl_name, opcode, line_num, opid, c_step, latency, opnd_num, bw_out, bw_0, bw_1, \
                        bw_2, m_delay, from_node_set, to_node_set, instruction = \
                        int(row[0]), row[1], row[2], row[3], row[4], row[5], row[6], row[7], row[8], row[9], row[10], \
                        row[11], row[12], row[13], row[14], row[15], row[16]
                    cdfg_fsm_dict[nodeid] = cdfg_fsm_node(nodeid=nodeid, name=name, rtl_name=rtl_name, opcode=opcode,
                                                          line_num=line_num, opid=opid,
                                                          c_step=c_step, latency=latency, opnd_num=opnd_num,
                                                          bw_out=bw_out, bw_0=bw_0, bw_1=bw_1, bw_2=bw_2,
                                                          m_delay=m_delay, from_node_set=from_node_set,
                                                          to_node_set=to_node_set, instruction=instruction)
...
def act_trace(in_dir, out_dir, inv_num):
    ########################
    with open('{}/final/global_dict.csv'.format(in_dir), 'r') as rfile:
    ...
    ########################
    edge_act_dict = dict()
    with open('{}/../preprocess/final/edge_obj_dict.csv'.format(out_dir), 'r') as fedge:
    ...
    with open('{}/edge_act_dict.csv'.format(out_dir), 'w+', newline='') as wfile:
        ...
        writer.writerow(
            ['edge_id', 'edge_src_id', 'edge_dst_id', 'src_id', 'dst_id', 'latency', 'op_cnt', 'hd_0', 'hd_1', 'hd_2'])
...
def get_opnd_index(opcode, opnd, instruction):
    ...
    elif opcode in op_act_select:
        # one situation is that the op connects to the mux signal of 'select', we neglect this case
...
def act_annotate(in_dir, out_dir, DG, inv_num):
    ...
    for nodeid in CG.nodes:
        ...
        elif opcode not in ['phi', 'fexp', 'getelementptr', 'alloca']:
            print('CHECK: act_annotate: graph nodeid = {} with opcode = {} not found in node_act_dict'.format(nodeid,
                                                                                                              opcode))
        ...
        for item in reg_flow:
            ...
            if is_str_num(dst_node_id) and int(dst_node_id) in cdfg_fsm_dict:
                ...
                if opnd_index != -1:  # for multiple reg_flow, find the corresponding interface and break the loop
                    edge_act = edge_act_dict[edge_id].hd_list[opnd_index] if opnd_index != -1 and opnd_index < len(
                        edge_act_dict[edge_id].hd_list) else 0.0
        ...
        if DG.nodes[nodeid]['opcode'] in ['load', 'read', 'phi']:
            for pre_id in list(DG.predecessors(nodeid)):
                for suc_id in list(DG.successors(nodeid)):
                    ...
                    DG.add_edge(pre_id, suc_id, src_activity=DG[pre_id][nodeid]['src_activity'],
                                dst_activity=DG[nodeid][suc_id]['dst_activity'],
                                src_act_ratio=DG[pre_id][nodeid]['src_act_ratio'],
                                dst_act_ratio=DG[nodeid][suc_id]['dst_act_ratio'],
...
def graph_node_feat_to_file(out_dir, DG):
    with open('{}/graph_node_feature.csv'.format(out_dir), 'w+', newline='') as wfile:
        writer = csv.writer(wfile)
        title_1 = ['nodeid', 'node_id', 'optype', 'opcode', 'latency', 'delay', 'act_ratio', 'hd_0', 'hd_1', 'hd_2',
                   'hd_sum',  # 'hd_out', 'hd_in',
                   'hd_fan_in', 'hd_fan_out', 'merge_op_set', 'fan_in', 'fan_out']
        title_2 = ['rtl_id', 'rtl_name', 'lut', 'dsp', 'bram', 'ff', 'mem_words', 'mem_bits', 'mem_banks',
                   'mem_wxbitsxbanks', 'opnd_num',
...
def graph_edge_feat_to_file(out_dir, DG):
    with open('{}/graph_edge_feature.csv'.format(out_dir), 'w+', newline='') as wfile:
        writer = csv.writer(wfile)
        title = ['edge_id', 'src_id', 'dst_id', 'src_activity', 'dst_activity', 'src_act_ratio', 'dst_act_ratio',
                 'reg_flow']
        writer.writerow(title)
        for src_id, dst_id in DG.edges:
...
def feature_embed(in_dir, out_dir, inv_num):
    DG = nx.DiGraph(nx.drawing.nx_pydot.read_dot('{}/final/DG.dot'.format(in_dir)))
    ...
    if CG.number_of_edges() < PLOT_EDGE_LIMIT:
        df_graph_visualize2(out_dir, 'DG_0', act_plot=True)
