...
numeric_item = ['hd_fan_in', 'hd_fan_out', 'hd_sum', 'act_ratio', 'op_cnt', 'latency', 'delay', 'fan_in', 'fan_out',
                'lut', 'dsp', 'bram', 'ff', 'mem_words', 'mem_bits', 'mem_banks', 'mem_wxbitsxbanks', 'opnd_num',
                'bw_out', 'bw_0', 'bw_1', 'bw_2']
categ_item = ['optype', 'opcode']

optype_categ = [['arith'], ['logic'], ['arbit'], ['io'], ['memory'], ['others']]
opcode_categ = [['fadd'], ['fsub'], ['fmul'], ['fdiv'], ['fadd_fsub'],
                ['store'], ['load'], ['load_store'],
                ['add'], ['sub'], ['mul'], ['div'], ['sqrt'], ['dadd'], ['dmul'], ['dsub'], ['ddiv'],
                ['mux'], ['select'], ['alloca'], ['getelementptr'], ['read'], ['write'],
                ['others']]
...
def read_metadata(file_dir, prj_name):
    with open('{}/{}/preprocess/final/global_dict.csv'.format(file_dir, prj_name), 'r') as rfile:
...
def get_metadata_feature(file_dir, prj_list):
    ...
    for prj_name in prj_list:
        ...
        if not init:
            ...
            baseline_latency, baseline_lut, baseline_ff, baseline_dsp, baseline_bram = latency, hls_lut, hls_ff, \
                hls_dsp, hls_bram
            init = True
        ...
        metadata_dict[prj_name] = metadata(sample_name=prj_name, clock=hls_clock, max_latency=max_latency,
                                           min_latency=min_latency, avg_latency=latency, lut=hls_lut, ff=hls_ff,
                                           dsp=hls_dsp, bram=hls_bram, sf_lut=sf_lut, sf_ff=sf_ff, sf_dsp=sf_dsp,
                                           sf_bram=sf_bram, sf_latency=sf_latency)
...
def get_pwr(pwr_dir):
    ...
    with open('{}/power_measurement.csv'.format(pwr_dir), 'r') as rfile:
        ...
        next(reader)
        for row in reader:
            if not row[0] in pwr_dict:
                # the units of total power (uW) and dynamic power (mW) is not the same, should make an alignment
                pwr_dict[row[0]] = list([float(row[1]), float(row[3])])
            else:
...
def generate_dot(DG, optype_enc, opcode_enc, dot_store_path):
        ...
        for feat_item in numeric_item:
            if feat_item not in DG.nodes[nodeid]:
                ...
                if type(DG.nodes[nodeid][feat_item]) == int:
                    ...
                    print("ERROR: feat_item = {}, not with type str or int, with type {}".format
                          (feat_item, type(DG.nodes[nodeid][feat_item])))
                    raise AssertionError()

        c_optype = DG.nodes[nodeid]['optype']
        c_opcode = DG.nodes[nodeid]['opcode']
        if c_opcode == 'fsub_fadd':
            c_opcode = 'fadd_fsub'
        elif c_opcode == 'store_load':
            c_opcode = 'load_store'
    ...
    for srcid, dstid in DG.edges():
        pyg_DG[srcid][dstid]['edge_attr'] = [float(DG[srcid][dstid]['src_activity'].strip('\"')),
                                             float(DG[srcid][dstid]['src_act_ratio'].strip('\"')),
                                             float(DG[srcid][dstid]['dst_activity'].strip('\"')),
                                             float(DG[srcid][dstid]['dst_act_ratio'].strip('\"'))]
...
def generate_dataframe(DG, pwr_list, pwr_status, overall_attr, kernel_name, prj_name, df_store_path):
    ...
    data['overall'] = [overall_attr.clock, overall_attr.max_latency, overall_attr.min_latency, overall_attr.avg_latency,
                       overall_attr.lut, overall_attr.ff, overall_attr.dsp, overall_attr.bram, overall_attr.sf_lut,
                       overall_attr.sf_ff, overall_attr.sf_dsp, overall_attr.sf_bram, overall_attr.sf_latency]
...
if __name__ == "__main__":
    ...
    parser.add_argument('--objective', help="power prediction objective: total/dynamic",
                        action='store', default='total')
    parser.add_argument('--prj_dir', help="directory of the output from the feature extraction stage",
                        action='store', default='./')
    parser.add_argument('--pwr_dir', help="directory of the file storing the power measurement",
                        action='store', default='./')
    parser.add_argument('--out_dir', help="directory to store the graph samples for training/testing",
                        action='store', default='./')
    ...
    tmp_list = list()
    for i in range(0, len(prj_list)):
        if re.match('prj_[0-1]', prj_list[i]):
            tmp_list.append(prj_list[i])
    prj_list = tmp_list
    ...
    for i, prj_name in enumerate(prj_list):
        if prj_name not in pwr_dict:
            ...
            pyg_DG = generate_dot(c_DG, optype_enc, opcode_enc, '{a}/dot/{b}/{b}_{c}.dot'.format(a=cur_dir,
                                                                                                 b=kernel_name,
                                                                                                 c=prj_name))
            generate_dataframe(pyg_DG, pwr_dict[prj_name], pwr_status[objective], metadata_dict[prj_name],
                               kernel_name, prj_name, '{a}/pyg_pt/{b}/{b}_{c}.pt'.format(a=cur_dir, b=kernel_name,
                                                                                         c=prj_name))
            dataframe = torch.load('{a}/pyg_pt/{b}/{b}_{c}.pt'.format(a=cur_dir, b=kernel_name, c=prj_name))
            # try loading to see whether successfully written or not
            dataframe_list.append(dataframe)
            print('processing #{} - {}/{} @ {}: {}'.format(processed_cnt, i+1, prj_cnt, time.asctime(
                time.localtime(time.time())), dataframe))

    torch.save(dataframe_list, '{a}/{b}.pt'.format(a=out_dir, b=kernel_name))

    print('{}: finished processing {}/{} files at: {}'.format(kernel_name, processed_cnt, prj_cnt,
